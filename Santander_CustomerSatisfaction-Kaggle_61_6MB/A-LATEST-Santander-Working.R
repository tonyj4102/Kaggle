...
# == Base                          [546]  train-auc:0.939935+0.003522	test-auc:0.823051+0.051957
# ==remove constant, zero variance [513]	train-auc:0.939752+0.002754	test-auc:0.819140+0.039505
# == zero feature                  [583]	train-auc:0.943103+0.004025	test-auc:0.816405+0.042485
# == no column by sample           [654]	train-auc:0.946713+0.002295	test-auc:0.818519+0.021026

# == 2 + subset k-means         auc =0.75

# == full set 100%, not13%, Base  

## Manual parameters booster = "gbtree", max.depth = 7, eta=0.125, nrounds = 30,nthread = 2, objective = "binary:logistic")
...
###  Preliminary feature engineering using Clustering: K-means
...
train <- subset(train, cluster == 2)
#train$cluster<-NULL


# take only 0.13
set.seed(113)  #15071
spl = sample(nrow(train), 0.13*nrow(train))
Strain = train[spl,]
...
bst = xgboost(data = as.matrix(trainMatrix), 
                label = y, 
                booster = "gbtree", 
                max.depth = 6,#7, 
                eta=0.01,# 0.125, 
...
# Feature Importance and Tree Diagrams

#Generate trees                          
Trees = xgb.model.dt.tree(dimnames(SStrain$data)[[2]], model = bst)
View(Trees)
Trees = xgb.model.dt.tree(dimnames(SStrain$names)[[2]], model = bst)
View(Trees)
